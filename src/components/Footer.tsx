      ...
      <div className="border-b border-border">
        <div className="container mx-auto px-4 py-12 md:py-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {properties.map((property, index) => <div key={index} className="bg-card rounded-lg p-6 flex flex-col h-full border border-border hover:border-wine-red/50 transition-colors">
                <h3 className="font-semibold text-lg mb-1 text-wine-red">
                  {property.name}
                </h3>
                {property.location && <p className="text-sm text-muted-foreground mb-3">
                    {property.location}
                  </p>}
                ...
                <div className="space-y-2 mb-4 flex-grow">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <a href={`tel:${property.phone.replace(/\s/g, "")}`} className="text-foreground underline hover:text-wine-red transition-colors">
                      {property.phone}
                    </a>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <Mail className="w-4 h-4 flex-shrink-0 text-wine-red" />
                    <a href={`mailto:${property.email}`} className="text-foreground underline hover:text-wine-red transition-colors">
                      {property.email}
                    </a>
                  </div>
                </div>

                {property.description && <p className="text-xs text-muted-foreground mb-4">
                    {property.description}
                  </p>}

                <div className="space-y-2">
                  {property.website.startsWith("http") ? <a href={property.website} target="_blank" rel="noopener noreferrer" className="block w-full">
                      <Button variant="default" size="sm" className="w-full bg-wine-red hover:bg-wine-red-dark text-white">
                        Zur Website
                        <ExternalLink className="w-3 h-3 ml-1" />
                      </Button>
                    </a> : <Link to={property.website} className="block w-full">
                      <Button variant="default" size="sm" className="w-full bg-wine-red hover:bg-wine-red-dark text-white">
                        Zur Website
                      </Button>
                    </Link>}
          ...
          <p className="text-center text-sm text-muted-foreground mt-8">
            Ein Verbund der{" "}
            <a href="https://www.birnbaum-hotels.de" target="_blank" rel="noopener noreferrer" className="text-wine-red underline hover:text-wine-red-light transition-colors">
              Birnbaum Hotels
            </a>
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-3">
                <img src={ritterLogo} alt="Ritter St. Georg Logo" title="Ritter St. Georg Logo" className="h-12 w-auto object-contain" />
                <div>
            ...
            <div className="space-y-4">
              <div className="pt-4 border-t border-border">
                ...
                <img src={birnbaumLogo} alt="Birnbaum Hotels & Restaurants" title="Birnbaum Hotels & Restaurants" className="h-16 w-auto object-contain hover:opacity-80 transition-opacity mb-4" />
              </div>
        ...
        <div className="container mx-auto px-4 py-5">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-neutral-400">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <span className="hidden md:inline">·</span>
              <button type="button" onClick={() => setCookieDialogOpen(true)} className="underline hover:text-wine-red transition-colors">
